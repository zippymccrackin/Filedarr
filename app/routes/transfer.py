...
async def receive_status(transfer_id):
    ...
    # See if this sequence number is newer than what we have stored
    existing = None
    if (existing := load_transfer(transfer_id)):
        if existing.get("sequence", 0) >= data.get("sequence", 0):
            return jsonify({"status": "ignore"})  # Ignore out-of-order updates
